...
template <typename Tv> struct Vertex { 
    ...
    Vertex ( Tv const& d = ( Tv ) 0 ) : 
       ...
       dTime ( -1 ), fTime ( -1 ), parent ( -1 ), height(-1) {}
 };
...
private:
	...
	void reset() { //所有顶点、边的辅助信息复位
       for ( int i = 0; i < n; i++ ) { //所有顶点的
          ...
          parent ( i ) = -1; height ( i ) = -1; //（在遍历树中的）父节点，高度
...
// 顶点的基本操作：查询第i个顶点（0 <= i < n）
	...
	void updateHeight ( int i){ //更新顶点i在遍历树中的高度
		int max = -1;
		for ( int j = firstNbr ( i ); -1 < j ; j = nextNbr ( i, j ) ){
			if ( TREE == type( i, j ) && j != parent( i ) &&max < height( j )) {
				max = height( j );
			}
		}
		height(i) = max + 1;
	...
	void updateHeightAbove ( int i){ //更新顶点i和i所有祖先顶点在遍历树中的高度
		do{ // 顶点父亲存在
			updateHeight( i );
			i = parent(i);
		}while(i > -1);
